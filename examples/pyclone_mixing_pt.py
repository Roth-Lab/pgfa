...
#     Z_true = Z_true[idxs]
    ...
    engine = ParallelTemperingEngine(model, model_updater, ladder_power=3.0, num_chains=num_chains)
    ...
    while timer.elapsed < time:            
        if iter_idx % 1 == 0:          
            ...
            print([engine.params[i].K for i in range(num_chains)])
...
class ParallelTemperingEngine(object):
    ...
    def __init__(self, model, model_updater, ladder_power=1.0, num_chains=10):
        ...
        self.params = [model.get_default_params(model.data, model.feat_alloc_dist) for _ in range(num_chains)]

...
def main():  
    ...
    idxs = np.random.permutation(data_df.shape[0])[:N]
    
    data_true = data_df.iloc[idxs].values
...
#     model.params.r_prior = np.array([10, 0.01])
     
#     model.params.S_prior = np.array([10, 0.01])
     
#     model.r = np.nansum(data, axis=1)
#     
#     model.S = data_true[:K]
    
    model.params.S = sigs_df.values + 1e-6
    
    model.params.S = model.params.S * (1 / model.params.S.max(axis=1)[:, np.newaxis])
...
#                 compute_error(data_test, model)
                log_predictive(data, data_true, model.params)
            )
...
def get_model(data, ibp=False, K=None):
    ...
    return pgfa.models.lnba.Model(data, feat_alloc_dist)
...
def get_model_updater(feat_alloc_updater_type='g', annealing_power=0, ibp=True, mixed_updates=False, num_particles=20):
    ...
    return pgfa.models.lnba.ModelUpdater(feat_alloc_updater)
...
def log_predictive(data, data_true, params):
    N, D = data.shape
    
    M = (params.Z * params.V) @ params.S
    
    log_p = 0 
    
    for n in range(N):
        for d in range(D):
            if not np.isnan(data[n, d]):
                continue
            
            log_p += pgfa.models.lnba.poisson_log_pdf(data_true[n, d], M[n, d])            

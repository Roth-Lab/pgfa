...
def simulate_data(params, mean_depth=1000, prop_missing=0):
    pi = params.W @ params.S + 1e-3

    pi = pi / np.sum(pi, axis=1)[:, np.newaxis]

    depth = scipy.stats.poisson.rvs(mean_depth, size=params.N)

    data_true = np.zeros((params.N, params.D), dtype=np.float)

    for n in range(params.N):
        data_true[n] = scipy.stats.multinomial.rvs(depth[n], pi[n])
...
def simulate_params(K, alpha=1, D=96, N=100):
    ...
    V = scipy.stats.gamma.rvs(1, scale=1, size=(N, K))

    S = scipy.stats.dirichlet.rvs(np.ones(D), size=K)

    return Parameters(alpha, np.ones(2), S, np.ones(D), V, np.ones(2), Z)
...
#         update_S_prior(model)

#         update_V_random_grid(model, num_points=10)
...
class DataDistribution(pgfa.models.base.AbstractDataDistribution):
    ...
    def log_p(self, data, params):
        return self.temp * _log_p(params.S, params.V, data, params.Z.astype(float))

    def log_p_row(self, data, params, row_idx):
        return self.temp * _log_p_row(params.S, params.V[row_idx], data[row_idx], params.Z[row_idx].astype(float))
